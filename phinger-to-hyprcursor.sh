...
for i in $(seq 0 "$(($(jq '.variants | length' cursor-theme.json) - 1))"); do
  ...
  currentTheme="$(jq -r ".variants[$i].name" <<<"$JSON")"
...
cursors_directory = cursors
  ...
  for c in $(seq 0 "$(($(jq ".variants[$i].cursors | length" <<<"$JSON") - 1))"); do
    ...
    cursorName="$(jq -r ".variants[$i].cursors[$c].name" <<<"$JSON")"
    # get the SVG filename without the [light|dark]/ prefix
    cursorFile="$(jq -r ".variants[$i].cursors[$c].sprites[0].file" <<<"$JSON" | cut -d/ -f2)"

    # rg for the first SVG in sprites[] & get the hotspot
    hotspot_x="$(rg "<rect id=\"center" "$cursorFile" | rg -Po '(?<= x=\")([0-9]*)' || echo 0)"
    hotspot_y="$(rg "<rect id=\"center" "$cursorFile" | rg -Po '(?<= y=\")([0-9]*)' || echo 0)"
...
" >"$CURSORDIR/$currentTheme/cursors/$cursorName/meta.hl"
      ...
      centerx="$(awk 'BEGIN{sum=0;count=0}match($0, /id="Vector(_[0-9]*)?" d="M[0-9.]*/){match($0,/M[0-9\.]+/);count+=1;sum+=substr($0,RSTART+1,RLENGTH)}END{print sum/count}' "$cursorFile")"
      centery="$(awk 'BEGIN{sum=0;count=0}match($0, /id="Vector(_[0-9]*)?" d="M[0-9.]* [0-9.]*/){match($0,/ [0-9\.]+/);count+=1;sum+=substr($0,RSTART+1,RLENGTH)}END{print sum/count}' "$cursorFile")"
      # get the angle, duration & ease from the json file
      rotationDegrees="$(jq -r ".variants[$i].cursors[$c].sprites[0].animations[0].instructions[1].args.degrees" <<<"$JSON")"
      duration="$(jq -r ".variants[$i].cursors[$c].sprites[0].animations[0].instructions[0].args.duration" <<<"$JSON")"
      totalFrames=60
      ease="$(jq -r ".variants[$i].cursors[$c].sprites[0].animations[0].instructions[0].args.ease" <<<"$JSON")"
